<template>
  <!-- 施工咨询列表 -->
  	<div class="construction_counsel_list">
		<div class="bgc">
		...
		<div class="zxw">
			<el-card :body-style="{ padding: '0px' }" v-for="(item, index) in pageResult.rows" :key="index">
				<div style="padding: 14px">
					...
					<div class="huifu" >咨询解答:{{ item.reply }}</div>
					<div class="huifu" v-if="item.evaluation!=null">服务评价:{{ item.evaluation }}</div>
					<div class="pjhf">
						<el-button class="pjhfhuigu" v-if=" judgeRoleByUserName(username) == 1 && item.is_replied == 0" type="text" @click="openReplyDialog(item.id)">回复</el-button>
						<el-button class="pjhfhuigu" v-if="judgeRoleByUserName(username) == 0 && !item.evaluation" type="text" @click="openEvaluationDialog(item.id)">评价</el-button>
					</div>
				</div>
			</el-card>
			<el-pagination
